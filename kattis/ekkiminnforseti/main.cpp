...
int main() {
	...
	cin>>n>>m;
	forn(i,m) cin>>names[i+1];
	input.assign(n,vi(m));
	forn(i,n) {
		forn(j,m) cin>>input[i][j];
		votes[input[i][0]].eb(i,0);
	}
	forn(i,m) candidates.insert(mp(votes[i+1].size(),-i-1));

	forn(i,m-1) {
		auto it2=candidates.rbegin();
		if((it2->fi)*2>n) {
			cout<<names[-(it2->se)]<<'\n';
			return 0;
		}
		...
		removed[-(it->se)]=1;
		changes.clear();
		for(auto &p: votes[-(it->se)]) {
			while(removed[input[p.fi][p.se]]) ++p.se;
			votes[input[p.fi][p.se]].pb(p);
			++changes[input[p.fi][p.se]];
		}
		candidates.erase(it);

		for(auto &p: changes) {
			ii prev=mp(votes[p.fi].size()-p.se,-p.fi);
			candidates.erase(lower_bound(candidates.begin(),candidates.end(),prev));
			candidates.insert(mp(votes[p.fi].size(),-p.fi));
		}

	}

	forn(i,m) if(!removed[i+1]) {
		cout<<names[i+1]<<'\n';
		break;
	}
