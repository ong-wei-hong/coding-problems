...
int main() {
	...
	cin>>V>>E>>Q;
	forn(j,E) cin>>X>>Y,graph[X][Y]=!c;
	forn(j,Q) {
		cin>>i;
		switch(i) {
			case 1:
				forn(k,V+1) graph[k][V]=graph[V][k]=c;
				++V;
				break;
			case 2:
				cin>>X>>Y;
			...
			case 3:
				cin>>X;
				forn(k,V) graph[k][X]=graph[X][k]=c;
				break;
			case 4:
				cin>>X>>Y;
				if(t) graph[Y][X]=c;
	...
	cout<<V<<'\n';
	forn(j,V) {
		ll hash=0LL,curr_pow=1;
		int d=0;
		forn(k,V) {
			if(j==k) continue;
			bool v=t?graph[k][j]:graph[j][k];
			if(v!=c) {
				++d,hash=add(hash, mul(curr_pow,k));
				curr_pow=mul(curr_pow,7);
			}
		}
		cout<<d<<' '<<hash<<'\n';
	}
