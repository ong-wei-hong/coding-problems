...
void solve() {
	...
	forn(i,n-1) {
		cin>>a;
		forn(j,1001) if(dp[i][j].fi!=-1) {
			if(dp[i+1][j+a].fi==-1||max(j+a,dp[i][j].fi)<dp[i+1][j+a].fi)
				dp[i+1][j+a]=mp(max(j+a,dp[i][j].fi),j);
			if(j-a>=0&&(dp[i+1][j-a].fi==-1||dp[i][j].fi<dp[i+1][j-a].fi))
				dp[i+1][j-a]=mp(dp[i][j].fi,j);
		}
	}

	if(dp[n-1][0].fi==-1) cout<<"IMPOSSIBLE\n";
	else {
		for(int i=n-1,j=0;i>=0;--i) {
			ans[i]=(dp[i][j].se>j?'D':'U');
			j=dp[i][j].se;
		}
		ans[n]='\n';
		ans[n+1]='\0';
	...
	//forn(i,40) forn(j,1001) if(dp[i][j].fi!=-1) cout<<'('<<i<<','<<j<<"):("<<dp[i][j].fi<<','<<dp[i][j].se<<")\n";
}
