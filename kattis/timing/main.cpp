...
const int MAX_N=100;
int n,l,a,b,t;
ld c,v[MAX_N],u[MAX_N];
vector<unordered_set<int>> al;

struct Matrix { ld mat[MAX_N][MAX_N]; };
Matrix matMul(Matrix a, Matrix b) {
	Matrix ans;
	FORN(i,n)
		FORN(j,n) ans.mat[i][j]=0;
	FORN(i,n) FORN(k,n) {
		if(a.mat[i][k] == 0) continue;
		FORN(j,n) ans.mat[i][j]+=a.mat[i][k]*b.mat[k][j];
	}
	return ans;
}
...
Matrix matPow(Matrix base, int p) {
	Matrix ans;
	FORN(i,n) FORN(j,n) ans.mat[i][j] = (i==j);
	while(p) {
...
void solve() {
	...
	FORN(i,n) {
		c=1;
		FORN(j,n) c-=ans.mat[i][j];
		ans.mat[i][i]=c;
	}

	//FORN(i,n) FORN(j,n) cout<<ans.mat[i][j]<<" \n"[j==n-1];

	ans=matPow(ans,t);

	FORN(i,n) {
		u[i]=0;
		FORN(j,n) u[i]+=ans.mat[j][i]*v[j];
	}

	ld ans2=1e6;
	FORN(i,n) {
		ld curr=u[i];
		for(auto j: al[i]) curr+=u[j];
		ans2=min(ans2,curr);
	}
//	FORN(i,n) FORN(j,n) cout<<ans.mat[i][j]<<" \n"[j==n-1];
