...
int solve(int i, int j, int k) {
	...
	int d[5] {0,1,0,-1,0};
	FORN(dd,4) {
		int new_i=i+d[dd],new_j=j+d[dd+1];
		if(0<=new_i&&new_i<n&&0<=new_j&&new_j<n&&m[new_i][new_j]>m[i][j]) 
			memo=max(memo,solve(new_i,new_j,k+1));
	}
...
void solve() {
	...
	int maxi=INT_MIN;
	FORN(i,n) 
		maxi=max(maxi,max(solve(i,0,0),max(solve(0,i,0),max(solve(n-1,i,0),solve(i,n-1,0)))));
	cout<<maxi<<'\n';
