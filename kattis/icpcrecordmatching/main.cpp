...
vector<tuple<string,string,string>> v,u;
string s,a,b,c;

bool eq(string &s, string &t) {
	if(s.size()!=t.size()) return 0;
	forn(i,s.size()) if(tolower(s[i])!=tolower(t[i])) return 0;
	return 1;
}

bool cmp(tuple<string,string,string> t1, tuple<string,string,string> t2) {
	string s=get<0>(t1),t=get<0>(t2);
...
void solve() {
	...
	while(getline(cin,s)) {
		...
		bool add=1;
		forn(i,v.size()) 
			if(eq(get<0>(v[i]),c)||(eq(get<1>(v[i]),b)&&eq(get<2>(v[i]),a))) {
				v.erase(v.begin()+i);
				add=0;
				break;
			}
	...
	if(v.size()==0&&u.size()==0) cout<<"No mismatches.\n";
	else {
		sort(all(v),cmp);
		sort(all(u),cmp);

		for(auto &t: v) cout<<"I "<<get<0>(t)<<' '<<get<1>(t)<<' '<<get<2>(t)<<'\n';
		for(auto &t: u) cout<<"O "<<get<0>(t)<<' '<<get<1>(t)<<' '<<get<2>(t)<<'\n';
	}
