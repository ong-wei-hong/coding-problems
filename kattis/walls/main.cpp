...
bool test(int a, int b, int c, int d) {
	pair<double,double> ps[4] {{-l/2.0,0},{l/2.0,0},{0,-w/2.0},{0,w/2.0}};
	bool bo[4]{0,0,0,0};
	int ar[4]{a,b,c,d};
	for(auto i: ar) forn(j,4) if(i!=-1&&sqrt(pow(ps[j].fi-v[i].fi,2)+pow(ps[j].se-v[i].se,2))<=r) bo[j]=1;
	return bo[0]&&bo[1]&&bo[2]&&bo[3];
}

void solve() {
	forn(i,n) if(test(i,-1,-1,-1)) {
		cout<<"1\n";
		return;
	}

	forn(i,n) for(int j=i+1;j<n;++j) if(test(i,j,-1,-1)) {
		cout<<"2\n";
		return;
	}

	forn(i,n) for(int j=i+1;j<n;++j) for(int k=j+1;k<n;++k) if(test(i,j,k,-1)) {
		cout<<"3\n";
		return;
	}

	forn(i,n) for(int j=i+1;j<n;++j) for(int k=j+1;k<n;++k) for(int l=k+1;l<n;++l) if(test(i,j,k,l)) {
		cout<<"4\n";
		return;
	}
