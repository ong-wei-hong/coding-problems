...
int m,n,t,C[31][31],a,dp[10001][31];
vi v;
...
void solve() {
	...
	forn(i,t+1) memset(dp[i],0,sizeof(dp[i]));
	dp[0][0]=1;
	for(auto i: v) for(int j=t-i;j>=0;--j) forn(k,n) dp[j+i][k+1]+=dp[j][k];

	cout<<dp[t][n]<<" : "<<C[m][n]-dp[t][n]<<'\n';
}
