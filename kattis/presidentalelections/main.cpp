...
int n,d,c,f,u,sumD,currD,dp[2017];
vii v;
...
int main() {
	...
	forn(i,n) {
		...
		sumD+=d;
		if(c>f+u) currD+=d;
		else if(c+u>f) v.eb(d,((f+u+c)/2+1)-c);
	}
	for(auto &p: v) for(int i=2016-p.fi;i>=0;--i) if(i==0||dp[i]) {
		if(dp[i+p.fi]==0) dp[i+p.fi]=p.se+dp[i];
		dp[i+p.fi]=min(dp[i+p.fi],dp[i]+p.se);
	}
	int mini=INT_MAX;
	if(currD>=sumD/2+1) {
		cout<<"0\n";
	...
	for(int i=(sumD/2+1)-currD;i<=2016;++i) if(dp[i]) mini=min(mini,dp[i]);
