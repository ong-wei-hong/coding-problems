...
int r,c,a[MAX_R][MAX_R],ans[2501],n,x;
bool visited[MAX_R][MAX_R];

int dfs(int low, int high, int i, int j) {
	int k=a[i][j];
	if(visited[i][j]||k<low||k>high) return 0;
	visited[i][j]=1;
	int d[5] {0,1,0,-1,0};
	int ans=1;
	FORN(a,4) {
		int new_i=i+d[a],new_j=j+d[a+1];
		if(0<=new_i&&new_i<r&&0<=new_j&&new_j<c) ans+=dfs(low,high,new_i,new_j);
	}
	return ans;
}
...
void solve(int low,int high) {
	...
	int curr=0;
	FORN(i,r) FORN(j,c) {
		curr=max(dfs(low,high,i,j),curr);
		if(curr>0) ans[curr]=ans[curr]==-1?high-low:min(ans[curr],high-low);
	}
}
...
void solve() {
	...
	FORN(low,100) for(int high=low;high<100;++high) {
		solve(low,high);
	}
	
	for(int k=r*c;k>0;--k) ans[k-1]=ans[k-1]==-1?ans[k]:min(ans[k-1],ans[k]);
