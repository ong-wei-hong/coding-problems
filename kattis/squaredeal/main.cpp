...
bool solve(ii b, int k) {
	return (b.fi==a[k].fi&&b.fi==(b.se+a[k].se)) ||
		(b.fi==a[k].se&&b.fi==(b.se+a[k].fi)) ||
		(b.se==a[k].fi&&b.se==(b.fi+a[k].se)) ||
		(b.se==a[k].se&&b.se==(b.fi+a[k].fi));
}

bool solve(int i, int j, int k) {
	return (a[i].fi==a[j].fi&&solve(mp(a[i].fi,a[i].se+a[j].se),k)) ||
		(a[i].fi==a[j].se&&solve(mp(a[i].fi,a[i].se+a[j].fi),k)) ||
		(a[i].se==a[j].fi&&solve(mp(a[i].se,a[i].fi+a[j].se),k)) ||
		(a[i].se==a[j].se&&solve(mp(a[i].se,a[i].fi+a[j].fi),k));
}

int main() {
	ios::sync_with_stdio(0),cin.tie(0),cout.tie(0);
	forn(i,3) cin>>a[i].fi>>a[i].se;
	if(solve(0,1,2)||solve(0,2,1)||solve(1,2,0)) cout<<"YES\n";
	else cout<<"NO\n";
}
