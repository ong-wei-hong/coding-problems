...
ld dp[200000],v[100000],prev_maxi;
tuple<ld, ld, int, int> u[100000];
int l,p;
queue<int> q;
bool b[200000];
...
ld solve(int i) {
	...
	auto [p1,p2,v1,v2] = u[i-l];
	memo = p1 * solve(v1) + p2 * solve(v2) + (1-p1-p2) * prev_maxi;
	return memo;
}
...
void solve() {
	...
	while(1) {
		ld maxi=-1;
		FORN(i,l+p) if(!b[i]) maxi=max(maxi,solve(i));
		if(abs(maxi-prev_maxi) < 1e-6) {
			prev_maxi=maxi;
			break;
		}
		prev_maxi=maxi;
	...
	cout<<fixed<<setprecision(10)<<prev_maxi<<'\n';
}
