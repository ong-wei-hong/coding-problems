...
void solve() {
	...
	forn(i,n) {
		int a=v[i];
		int j=lower_bound(all(dp),mp(a,-1))-dp.begin();
		if(j==dp.size()) dp.eb(a,i);
		else if(dp[j].fi!=a) dp[j]=mp(a,i);
		if(j!=0) prv[i]=dp[j-1].se;
	}
	
	ans.clear();
	for(int i=dp.back().se;i!=-1;i=prv[i]) ans.pb(v[i]);
	reverse(all(ans));
