...
void solve() {
	...
	for(int start=1;start<=10;++start) for(int end=start;end<=10;++end) {
		int mini=INT_MAX;
		for(int i=start;i<=end;++i) mini=min(mini,a[i]);
		if(mini>a[start-1]&&mini>a[end+1]) ++ans;
	}
