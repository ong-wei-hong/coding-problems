...
int n,a,d,f[MAX_N];
ii jumps[MAX_N],ans[MAX_N];
bool source[MAX_N];

void solve(int v) {
	if(jumps[v].se!=-1) return;

	if(f[v]==-1) jumps[v]={v,0};
	else {
		solve(f[v]);
		jumps[v]={jumps[f[v]].fi,jumps[f[v]].se+1};
		source[f[v]]=1;
	}
}
...
void solve() {
	...
	FORN(i,n) if(jumps[i].se==-1) solve(i);
	FORN(i,n) 
		if(jumps[i].fi!=-1&&!source[i]&&(ans[jumps[i].fi].se==-1||jumps[i].se<ans[jumps[i].fi].se))
			ans[jumps[i].fi]={i,jumps[i].se};
		else if(jumps[i].fi!=-1&&!source[i]&&jumps[i].se==ans[jumps[i].fi].se)
			ans[jumps[i].fi].fi=min(ans[jumps[i].fi].fi,i);

	cin>>a;
	while(a--) {
		cin>>d;
		cout<<ans[jumps[d].fi].fi<<'\n';
	}
