...
const int MAX_N=11;
int xSize,ySize,x,y,n,dp[MAX_N][1<<(MAX_N-1)];
vii v;

int dist(int i, int j) {
	return abs(v[i].fi-v[j].fi)+abs(v[i].se-v[j].se);
}

int solve(int i,int visited) {
	if(visited==0) {
		return dist(i,0);
	}
	...
	ans=INT_MAX;
	for(int m=visited;m;m-=LSOne(m)) {
		int j=LSOne(m),k=__builtin_ctz(j)+1;
		ans=min(ans,dist(i,k)+solve(k,visited^j));
	}
	return ans;
}

void solve() {
	cin>>xSize>>ySize>>x>>y>>n;
	v.resize(n+1);
	v[0]=mp(x,y);
	forn(i,n) cin>>v[i+1].fi>>v[i+1].se;
