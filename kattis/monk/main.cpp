...
void solve() {
	...
	v.resize(a);
	u.resize(d);
	for(auto &p: v) cin>>p.fi>>p.se,target+=p.fi;
	for(auto &p: u) cin>>p.fi>>p.se;

	double lb=0,ub=5e5;
	while((ub-lb)>0.000001) {
		double m=(lb+ub)/2,e=0,curr=m;
		forn(i,a) 
			if(curr>=v[i].se) e+=v[i].fi,curr-=v[i].se;
			else e+=(curr/v[i].se)*v[i].fi,curr=0; 
		curr=m;
		forn(i,d) 
			if(curr>=u[i].se) e+=u[i].fi,curr-=u[i].se;
			else e+=(curr/u[i].se)*u[i].fi,curr=0;
		if(e>=target) ub=m;
		else lb=m;
