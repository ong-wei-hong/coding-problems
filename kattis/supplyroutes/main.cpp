...
int main() {
	...
	FORN(i,Q) {
		...
		if(queries[i][0]==0) streets.erase(mp(queries[i][1],queries[i][2]));
	}

	ufds=UnionFind(N);
	for(auto &p: streets) ufds.unionSet(p.fi,p.se);
	for(int i=Q-1;i>=0;--i) if(queries[i][0]==0) {
		ufds.unionSet(queries[i][1],queries[i][2]);
	} else {
		if(ufds.isSameSet(queries[i][1],queries[i][2])) ans.pb("safe\n");
		else ans.pb("unsafe\n");
	}
