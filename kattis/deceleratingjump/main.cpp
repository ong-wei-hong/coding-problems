...
int n;
ll dp[3000][3000],a[3000];

ll solve(int curr, int jump) {
	if(curr==n-1) return a[n-1];
	jump=min(jump,n-1-curr);
	ll &ans = dp[curr][jump];
	if(ans != LLONG_MIN) return ans;
	
	if(jump>1) ans=solve(curr,jump-1);
	ans=max(ans,solve(curr+jump,jump)+a[curr]);

	return ans;
}

void solve() {
	FORN(i,3000) FORN(j,3000) dp[i][j]=LLONG_MIN;
	cin>>n;
	FORN(i,n) cin>>a[i];
