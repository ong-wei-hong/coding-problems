...
ll dfs1(int cur,int prv,ll path_cost) {
	ll curr_cost = 1LL * trips[cur] * path_cost;
	dtrips[cur] += trips[cur];
	for(auto [nxt, cost]: al[cur]) if(nxt!=prv) 
		curr_cost += dfs1(nxt,cur,path_cost+cost),dtrips[cur]+=dtrips[nxt];
	return curr_cost;
}

void dfs2(int cur, int prv, ll curr_cost, ll weight) { 
	curr_cost += -weight * dtrips[cur] + weight * (sumtrips - dtrips[cur]);
	if(curr_cost < mincost) mincost = curr_cost, ans.clear();
	if(curr_cost == mincost) ans.pb(cur);

	for(auto [nxt, cost]: al[cur]) if(nxt != prv) dfs2(nxt,cur,curr_cost,cost);
}
