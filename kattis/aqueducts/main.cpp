...
typedef tuple<int, ll, ll, ld> edge2;
...
ld dist(ld x1, ld y1, ld x2, ld y2, ld h1, ld h2) {
...
void solve() {
	...
	FORN(i,n) for(int j=i+1;j<n;++j) {
		int a=i,b=j;
		if(get<2>(v[i])<get<2>(v[j])) swap(a,b);
		auto [x1,y1,h1]=v[a];
		auto [x2,y2,h2]=v[b];
		ld d = dist(x1,y1,x2,y2,h1,h2);
		if(h2<h1&&d<=q) 
			mcmf.add_edge(a+1,b+1,t,d);
	}

	for(auto i: source) mcmf.add_edge(0,i,1,0);
	for(auto i: sink) mcmf.add_edge(i,n+1,1,0);

	auto [mf, total_cost] = mcmf.mcmf(0,n+1);
	if(mf>=t) cout<<fixed<<setprecision(10)<<total_cost<<'\n';
