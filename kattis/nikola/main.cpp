...
int n,v[MAX_N+1],dp[MAX_N+1][MAX_N+1];
...
void solve() {
	...
	while(!q.empty()) {
		...
		int nxt=node-step;
		if(nxt>0&&dp[nxt][step]==-1) {
			dp[nxt][step]=dp[node][step]+v[nxt];
			q.emplace(-dp[nxt][step],nxt,step);
		}

		nxt=node+step+1;
		if(nxt<=n&&dp[nxt][step+1]==-1) {
			dp[nxt][step+1]=dp[node][step]+v[nxt];
			q.emplace(-dp[nxt][step+1],nxt,step+1);
		}
	}
}
