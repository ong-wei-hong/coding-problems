...
int main() {
	...
	FORN(i,10) {
		FORN(j,i+1) dp[j][1][i]=1;
		for(int l=2;l<=100;++l) {
			dp[0][l][i]=dp[0][l-1][i]+dp[1][l-1][i];
			dp[i][l][i]=dp[i][l-1][i]+dp[i-1][l-1][i];
			for(int j=1;j<i;++j) dp[j][l][i]=dp[j-1][l-1][i]+dp[j][l-1][i]+dp[j+1][l-1][i];
		}
	}
