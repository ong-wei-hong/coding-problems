...
void solve() {
	double lo=-pow(10,8),hi=pow(10,8);
	while(1) {
		double mid=(lo+hi)/2,currX=0;
		int currY=0;
		for(int i=0;currY<y;++i) {
			if(i==n||get<0>(v[i])>=y) currX+=mid*(y-currY),currY=y;
			else {
				currX+=mid*(get<0>(v[i])-currY);
				currY=get<0>(v[i]);
				currX+=mid*(get<2>(v[i]))*(min(get<1>(v[i]),y)-currY);
				currY=get<1>(v[i]);
			}
		}
		if(abs(currX-x)<0.0000001) {
