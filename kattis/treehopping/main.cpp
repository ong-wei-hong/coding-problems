...
int t,n,a,b,parent_a[4],parent_b[4],parent[100001],p[100000];
vector<vi> al;

void dfs(int curr, int prev) {
	parent[curr]=prev;
	for(auto i: al[curr]) if(i!=prev) dfs(i,curr);
}

void find_parent(int curr, int* parent_curr) {
	parent_curr[0]=curr;
	FORN(i,3) parent_curr[i+1]=parent[parent_curr[i]];
}

int solve2() {
	FORN(i,4) FORN(j,4-i)
		if(parent_a[i]&&parent_b[j]&&parent_a[i]==parent_b[j]) return 1;
	return 0;
}
...
void solve() {
	...
	int a=p[0];
	find_parent(a,parent_a);
	int valid=1;
	FORN(i,n-1) {
		b=p[i+1];

		find_parent(b,parent_b);

		bool found=0;
		valid&=solve2();
		if(!valid) break;
		a=b;
		FORN(i,4) parent_a[i]=parent_b[i];
	}
