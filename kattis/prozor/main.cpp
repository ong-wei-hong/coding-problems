...
string m[100];
int dp[101][101],r,s,k,ansX,ansY,ans;
...
int main() {
	...
	forn(i,r) forn(j,s) dp[i+1][j+1]+=dp[i][j+1]+dp[i+1][j]-dp[i][j]+(m[i][j]=='*');

	forn(i,r-k+1) forn(j,s-k+1) {
		int curr=dp[i+1][j+1]+dp[i+k-1][j+k-1]-dp[i+k-1][j+1]-dp[i+1][j+k-1];
		if(curr>ans) {
			ans=curr;
			ansX=i;
			ansY=j;
	...
	m[ansX][ansY]=m[ansX+k-1][ansY]=m[ansX][ansY+k-1]=m[ansX+k-1][ansY+k-1]='+';
	forn(i,k-2) m[ansX][ansY+1+i]=m[ansX+k-1][ansY+1+i]='-',m[ansX+i+1][ansY]=m[ansX+i+1][ansY+k-1]='|';
