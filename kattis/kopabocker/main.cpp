...
int N,M,prices[15][100],postage[15],K,F,I,P;

int solve(int mask) {
	int ans=0;
	for(int m=mask;m;m-=LSONE(m)) ans+=postage[__builtin_ctz(m)];

	FORN(i,N) {
		int curr=INT_MAX;
		for(int m=mask;m;m-=LSONE(m)) {
			int p=prices[__builtin_ctz(m)][i];
			if(p) curr=min(curr,p);
		}
		if(curr==INT_MAX) return INT_MAX;
		ans+=curr;
	}
...
void solve() {
	...
	int ans=INT_MAX;
	FORN(mask,(1<<M)) ans=min(ans,solve(mask));
	cout<<ans<<'\n';
