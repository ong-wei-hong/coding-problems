...
private:
	...
	vll sums;
public:
	UnionFind2(int n) : ufds(UnionFind(n)),p(vi(n)),sums(vll(n)) {
		forn(i,n) p[i]=i,sums[i]=i;
		count.resize(n,1);
	}

	void unionSet(int a, int b) {
		int x=ufds.findSet(p[a]),y=ufds.findSet(p[b]);
		if(ufds.isSameSet(x,y)) return;
		ufds.unionSet(x,y);
		sums[ufds.findSet(x)]=sums[x]+sums[y];
		count[ufds.findSet(x)]=count[x]+count[y];
	}
