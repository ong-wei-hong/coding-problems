...
bool check(int i, int j, int k) {
	bool b=1;
	forn(v,4) b&=(a[i][v]==a[j][v]&&a[i][v]==a[k][v])||(a[i][v]!=a[j][v]&&a[i][v]!=a[k][v]&&a[j][v]!=a[k][v]);
	return b;
}
...
int main() {
	...
	for(auto &s: a) cin>>s;
	bool b=1;
	forn(i,12) for(int j=i+1;j<12;++j) for(int k=j+1;k<12;++k) if(check(i,j,k)) b=0,cout<<i+1<<' '<<j+1<<' '<<k+1<<'\n';
	if(b) cout<<"no sets\n";
}
