...
int ans,n,k,m;
vector<unordered_set<string>> markets;
vector<string> items;
unordered_map<string,int> route1,route2;

void solve() {

}

void build_route(int dir, unordered_map<string,int> &route) {
	int currMarket=0,maxMarket=n,currItem=0,maxItem=m;
	if(dir==-1) currMarket=n-1,maxMarket=-1,currItem=m-1,maxItem=-1;

	while(currMarket!=maxMarket&&currItem!=maxItem) {
		if(markets[currMarket].find(items[currItem]) != markets[currMarket].end()) {
			route[items[currItem]]=currMarket;
			currItem+=dir;
		} else {
			currMarket+=dir;
		}
...
int main() {
	...
	build_route(1,route1);
	build_route(-1,route2);

	for(auto &s: items) if(route1.find(s)==route1.end()) {
		cout<<"impossible\n";
		return 0;
	}

	for(auto &p: route1) if(route2[p.fi]!=p.se) {
		cout<<"ambiguous\n";
		return 0;
	}
