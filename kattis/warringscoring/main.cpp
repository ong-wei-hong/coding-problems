...
int n,ans1Y,curr1Y,ans1N,curr1N,curr2Y,curr2N,ans2Y,ans2N;
string s;
...
void solve() {
	while(n--) {
		...
		if(s[0]=='Y') {
			++curr1Y;
			curr1N=0;
			ans1Y=max(ans1Y,curr1Y);

			++curr2Y;
			ans2Y=max(ans2Y,curr2Y-curr2N);
		} else {
			++curr1N;
			curr1Y=0;
			ans1N=max(ans1N,curr1N);

			++curr2N;
			ans2N=max(ans2N,curr2N-curr2Y);
		}
	}

	if(((ans1Y>ans1N)==(ans2Y>ans2N))&&((ans1Y==ans1N)==(ans2Y==ans2N))) cout<<"Agree\n";
	else cout<<"Disagree\n";
}
