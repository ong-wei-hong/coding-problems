...
void add_move(int i) {
	moves.pb(to_string(i));
}

void add_move(int i, int j) {
	moves.pb(to_string(i)+' '+to_string(j));
}
...
void solve() {
	...
	int x=a[0],y=a[1];
	if(n&1) add_move(x,a[2]),add_move(x),ans+=x+a[2];
	for(int i=(2+(n&1));i<n;i+=2)
		if(x+2*y+a[i+1]<2*x+a[i]+a[i+1]) {
			add_move(x,y),add_move(x),add_move(a[i],a[i+1]),add_move(y);
			ans+=x+2*y+a[i+1];
		} else {
			add_move(x,a[i]),add_move(x),add_move(x,a[i+1]),add_move(x);
			ans+=2*x+a[i]+a[i+1];
		}
	add_move(x,y);
	ans+=y;
