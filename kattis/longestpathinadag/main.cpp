...
int v,e,out[MAX_V],a,b;
ii nxt[MAX_V];
vi al[MAX_V],ral[MAX_V];
...
void solve() {
	...
	int start=-1,l=0;
	queue<int> q;
	FORN(i,v) if(out[i]==0) q.push(i);

	while(!q.empty()) {
		int i=q.front(); q.pop();
		nxt[i]={-1,1};
		for(auto j: al[i]) if(nxt[j].se>=nxt[i].se) nxt[i]={j,nxt[j].se+1};

		if(nxt[i].se>l) start=i,l=nxt[i].se;

		for(auto j: ral[i]) if(--out[j]==0) q.push(j);
	}

	cout<<l<<'\n';
	while(start!=-1) cout<<start+1<<' ',start=nxt[start].fi;
	cout<<'\n';
}
