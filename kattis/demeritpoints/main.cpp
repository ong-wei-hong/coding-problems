...
int t,n,prv,merit,demerit;

void printYear(int y) {
	cout<<y/10000<<'-'<<setfill('0')<<setw(2)<<right<<(y/100)%100<<'-'<<setfill('0')<<setw(2)<<right<<y%100;
}

void print(int y) {
	if(merit) printYear(y),cout<<' '<<merit<<" merit point(s).\n";
	else if(demerit) printYear(y),cout<<' '<<demerit<<" demerit point(s).\n";
	else printYear(y),cout<<" No merit or demerit points.\n";
}

void events_between(int y) {
	for(int currYear=prv;merit<5;) {
		if(demerit) {
			currYear+=10000;
			if(currYear>y) break;
			demerit=min(demerit/2,demerit-2);
			demerit=max(demerit,0);
			print(currYear);
		} else {
			currYear+=20000;
			if(currYear>y) break;
...
void solve() {
	...
	demerit+=n;
	if(merit&&demerit) 
		if(demerit>2*merit) demerit-=2*merit,merit=0;
		else merit-=(demerit+1)/2,demerit=0;
	print(t);
}
