...
int main() {
	...
	FORN(i,5) {
		FORN(j,2) {
			string s;
			cin>>s;
			ans[i][j+2]=(s=="_"?0:stoi(s));
		}
	}

	FORN(i,5) 
		FORN(v1,4) FORN(v2,4) if(v1!=v2&&ans[i][v1]&&ans[i][v2]) {
			r[v1][v2]=((ld)ans[i][v1])/ans[i][v2];
			r[v2][v1]=((ld)1)/r[v1][v2];
		}
	
	FORN(i,6)
		FORN(v1,4) FORN(v3,4) if(v1!=v3) FORN(v2,4) if(!r[v1][v3]&&r[v1][v2]&&r[v2][v3]) {
			r[v1][v3]=r[v1][v2]*r[v2][v3];
			r[v3][v1]=((ld)1)/r[v1][v3];
		}

	FORN(i,5) FORN(j,4) if(ans[i][j]) {
		FORN(k,4) if(k!=j&&r[k][j]&&!ans[i][k]) ans[i][k]=(int)round(r[k][j]*ans[i][j]);
		break;
	}

	FORN(i,5) FORN(j,2) if(ans[i][j]) cout<<ans[i][j]<<' ';
	else cout<<"_ ";
	cout<<'\n';
	FORN(i,5) FORN(j,2) if(ans[i][j+2]) cout<<ans[i][j+2]<<' ';
	else cout<<"_ ";
	cout<<'\n';
