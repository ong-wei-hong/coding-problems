...
bool visited[11],found;

void solve(int curr) {
	visited[curr]=1;
	int prev_s1=s1.size(),prev_s2=s2.size();
	s1+=v[curr].fi;
	s2+=v[curr].se;
	for(int i=min(prev_s1,prev_s2);i<min(s1.size(),s2.size());++i) if(s1[i]!=s2[i]) {
		s1.erase(prev_s1);
		s2.erase(prev_s2);
		visited[curr]=0;
		return;
	...
	if(s1.size()==s2.size()) {
		if(ans == ""||s1.size()<ans.size()||(s1.size()==ans.size()&&s1<ans)) ans=s1;
		s1.erase(prev_s1);
		s2.erase(prev_s2);
		visited[curr]=0;
		return;
