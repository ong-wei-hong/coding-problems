...
ld dist(point &p, ld x, ld y) {
	return sqrt(pow(p.x-x,2)+pow(p.y-y,2));
}

bool intersect(point &p, ld x, ld y) {
...
bool solve() {
	...
	for(auto &p: v) {
		if(intersect(p, x, y) ||
				intersect(p, x+w, y) ||
				intersect(p, x, y+h) ||
				intersect(p, x+w, y+h)) return false;

		if(x <= p.x && p.x <= x+w && ((y <= p.y-p.r && p.y-p.r <= y+h)||(y <= p.y+p.r && p.y+p.r <= y+h))) return false;
		if(y <= p.y && p.y <= y+h && ((x <= p.x-p.r && p.x-p.r <= x+w)||(x <= p.x+p.r && p.x+p.r <= x+w))) return false;
	}
