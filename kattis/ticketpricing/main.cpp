...
void solve() {
	...
	forn(i,w+1) {
		...
		forn(j,k)
			if(i==0) {
				int q_sold=min(n,q[j]),r=p[j]*q_sold;
				if(dp[i][n-q_sold].fi==-1||dp[i][n-q_sold].fi<r)
					dp[i][n-q_sold]=mp(r,p[j]);
				if(dp[i][n-q_sold].fi==r) dp[i][n-q_sold].se=min(dp[i][n-q_sold].se,p[j]);
			} else 
				forn(k,n+1) 
					if(dp[i-1][k].fi!=-1) {
						int q_sold=min(k,q[j]),r=dp[i-1][k].fi+q_sold*p[j];
						if(dp[i][k-q_sold].fi==-1||dp[i][k-q_sold].fi<r)
							dp[i][k-q_sold]=mp(r,dp[i-1][k].se);
						if(dp[i][k-q_sold].fi==r)
							dp[i][k-q_sold].se=min(dp[i][k-q_sold].se,dp[i-1][k].se);
					}
		}

	int ans1=0,ans2=0;
	forn(i,n+1) {
		if(dp[w][i].fi>ans1) ans1=dp[w][i].fi,ans2=dp[w][i].se;
		if(dp[w][i].fi==ans1) ans2=min(ans2,dp[w][i].se);
	}
	cout<<ans1<<'\n'<<ans2<<'\n';
}
