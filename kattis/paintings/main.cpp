...
void f(int i) {
	...
	forn(j,n) {
		...
		int x=a[i-1],y=j;
		if(x>y) swap(x,y);
		if(pairs.count(mp(x,y))) continue;
		b[j]=1;
		a[i]=j;
		f(i+1);
		b[j]=0;
...
void solve() {
	cin>>n;
	colors.resize(n); 
	for(auto &s: colors) cin>>s;
	cin>>m;
	pairs.clear();
	forn(i,m) {
		cin>>s1>>s2;
		int x=find(all(colors),s1)-colors.begin(),y=find(all(colors),s2)-colors.begin();
		if(x>y) swap(x,y);
		pairs.emplace(x,y);
	}
