...
int main() {
	...
	forn(i,s.size())
		if(s[i]=='(') st.push(i);
		else if(s[i]==')') v.eb(st.top(),i),st.pop();

	unordered_set<int> se;
	forn(i,1<<v.size()) if(i!=0) {
		se.clear();
		for(int j=0,k=i;k!=0;++j,k>>=1) if(k&1) se.insert(v[j].fi),se.insert(v[j].se);
		string t="";
		forn(j,s.size()) if(!se.count(j)) t+=s[j];
		ans.insert(t);
	}
