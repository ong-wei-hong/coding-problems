...
int graph1[1000][4],graph2[1000][4],visited[1000],n;
bool same=1,solved=0;
vi st;
...
void dfs(int i) {
	...
	if(i==n-1) {
		...
		solved=1;
		int curr=0;
		for(auto j: st) curr=graph2[curr][j];
		same&=(curr==n-1);
	}
...
int main() {
	...
	cin>>n;
	FORN(i,n) FORN(j,4) cin>>graph1[i][j],graph1[i][j]--;
	FORN(i,n) FORN(j,4) cin>>graph2[i][j],graph2[i][j]--;
	solve();
}
