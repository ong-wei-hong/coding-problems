...
int solve(int i) {
	...
	int ans=solve(i+1);
	for(auto t: u) {
		int w1=get<0>(t),h1=get<1>(t),x1=get<2>(t),y1=get<3>(t),
		    w2=get<0>(v[i]),h2=get<1>(v[i]),x2=get<2>(v[i]),y2=get<3>(v[i]);

		if(x1>x2) swap(x1,x2),swap(w1,w2);
		if(y1>y2) swap(y1,y2),swap(h1,h2);
		if(x2<x1+w1&&y2<y1+h1) return ans;
	}

	u.pb(v[i]);
	ans=max(ans,get<0>(v[i])*get<1>(v[i])+solve(i+1));
	u.pop_back();
	return ans;
}
...
int main() {
	...
	while(1) {
		...
		if(n==0) break;
		v.resize(n);
		forn(i,n) cin>>a>>b>>c>>d,v[i]={a,b,c,d};
		cout<<solve(0)<<'\n';
	}
}
