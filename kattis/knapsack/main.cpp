...
void solve() {
	...
	ans.clear();
	for(int i=n,j=c;i!=0&&j!=0;) {
		if(j>=v[i].se&&dp[i-1][j-v[i].se]+v[i].fi==dp[i][j]) ans.pb(i),j-=v[i].se;
		--i;
	}

	cout<<ans.size()<<'\n';
	forn(i,ans.size()) cout<<ans[ans.size()-i-1]-1<<" \n"[i==ans.size()-1];
	//cout<<"--\n";
	//forn(i,n+1) forn(j,c+1) cout<<dp[i][j]<<" \n"[j==c];
	//cout<<"--\n";
