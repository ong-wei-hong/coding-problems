...
int n,a[MAX_N],nxt[MAX_N],prv[MAX_N],mn;
bool d[MAX_N];

bool gd(int i) {
	if(i<1||i>n) return 0;
	return a[prv[i]]+1==a[i]||a[nxt[i]]+1==a[i];
}

void solve() {
	cin>>n;
	mn=INT_MAX;
	forn(i,n) cin>>a[i+1],nxt[i+1]=i+2,prv[i+1]=i,mn=min(mn,a[i+1]),d[i+1]=0;
	if(mn) {
		cout<<"NO\n";
		return;
	}
	...
	pq<ii> q;
	forn(i,n) if(gd(i+1)) d[i+1]=1,q.emplace(a[i+1],i+1);
	...
	while(q.size()) {
		...
		nxt[prv[i]]=nxt[i];
		prv[nxt[i]]=prv[i];
		if(!d[prv[i]]&&gd(prv[i])) q.emplace(a[prv[i]],prv[i]),d[prv[i]]=1;
		if(!d[nxt[i]]&&gd(nxt[i])) q.emplace(a[nxt[i]],nxt[i]),d[nxt[i]]=1;
	}
