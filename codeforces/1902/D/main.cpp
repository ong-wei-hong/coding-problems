...
void solve() {
	...
	if(it!=m.end()&&(it->se.front()<l||it->se.back()>=r)) {
		cout<<"YES\n";
		return;
	}
	it=m.find(mp(v[r].fi-x+v[l-1].fi,v[r].se-y+v[l-1].se));
	//cout<<"\nfinding "<<v[r].fi-x+v[l-1].fi<<' '<<v[r].se-y+v[l-1].se<<'\n';
	if(it!=m.end()) { auto it2=lower_bound(all(it->se),l); if(it2!=it->se.end()&&*it2<=r) {
			cout<<"YES\n";
			return;
		}
	}
