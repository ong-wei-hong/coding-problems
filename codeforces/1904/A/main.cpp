...
int a,b,xk,yk,xq,yq,p[5] {-1,1,1,-1,-1};
vii v,u;
...
void solve() {
	...
	v.clear();
	u.clear();
	forn(i,4) v.eb(xk+p[i]*a,yk+p[i+1]*b),u.eb(xq+p[i]*a,yq+p[i+1]*b);
	if(a!=b) forn(i,4) v.eb(xk+p[i]*b,yk+p[i+1]*a),u.eb(xq+p[i]*b,yq+p[i+1]*a);

	int ans=0;
	for(auto &p: v) for(auto &r: u) ans+=(p.fi==r.fi&&p.se==r.se);
	cout<<ans<<'\n';
