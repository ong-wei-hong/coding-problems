...
int n;
vi a,b,m,nextGreaterA,prevSmallerB;
vector<bool> bo;
...
void solve() {
	...
	stack<ii> st;
	nextGreaterA.resize(n);
	prevSmallerB.resize(n);
	bo.assign(n,0);
	forn(q,2) {
		m.assign(n+1,-1);
		st.push(mp(INT_MAX,n));
		forn(i,n) {
			while(st.top().fi<=a[n-1-i]) st.pop();
			nextGreaterA[n-1-i]=st.top().se;
			st.push(mp(a[n-1-i],n-1-i));
		}
		while(!st.empty()) st.pop();
		st.push(mp(INT_MIN,-1));
		forn(i,n) {
			while(st.top().fi>=b[i]) st.pop();
			prevSmallerB[i]=st.top().se;
			st.push(mp(b[i],i));
		}
		while(!st.empty()) st.pop();
		...
		forn(i,n) {
			m[a[i]]=i;
			if(m[b[i]]!=-1&&a[i]<=b[i]) bo[i]=bo[i]|(nextGreaterA[m[b[i]]]>i&&prevSmallerB[i]<m[b[i]]);
		}
		reverse(all(a));
		reverse(all(b));
		reverse(all(bo));
